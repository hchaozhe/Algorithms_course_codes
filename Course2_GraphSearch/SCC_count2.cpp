#include "Graph_Directed_CRH.hpp" // customized directed graph header files 
#include <algorithm>    // std::sort
#include <stack>
// This program uses non recurisve DFS search
// It does not suffer the memory issue that the DFS one may suffer
// unless you change the memory setting to allow usage of more memory. 
/
// see the following blog for how iterative approach was implemented with the finishing time 
// calculation. 
...
// Kosaraju's algorithm can be implemented using BFS too, but it is more complicated
//
...
// non recursive First DFS, go through the reversed edge to check the finishing time
// the larger finishing time, the 
...
// order the graph vertex according to finishing time, gives the order for the 
// second DFS search
...
// second DFS, follow the finishing time, startwith the latest one
// DFS according to the orignal direction, those who finished last, should be longs to the
// last SCC.
...
int main(int argc, char *argv[]){
    ...
    // to output the 5 largest SCC size
    // My answer is 434821,968,459,313,211 

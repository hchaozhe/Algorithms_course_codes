...
// [CRH] This is specifically for the Dijstra algorithm
...
class MinHeap
{
	...
	int *pos; // pointer to array of elements corresponds to position in the heap
  ...
  public:
	...
	// Returns the position of any vertex from min heap
	int getpos(int v) { return pos[v]; }
	...
	bool empty();
	// A utility function to check if a given vertex v is in the 
	// min Heap or hot
...
void MinHeap::printheap(){
	...
	for (i = 0;i<heap_size;i++){
		cout << varr[i] << " " << pos[i] << endl;
	}  
	for (i = heap_size;i<capacity;i++){
		cout << "  " << " " << pos[i] << endl;
	} 
}
...
int MinHeap::extractMin()
{
	...
	pos[varr[0]] = heap_size-1; 
	pos[varr[heap_size-1]] = 0;
...
if (pos[v]<heap_size) return true;
return false;
}
